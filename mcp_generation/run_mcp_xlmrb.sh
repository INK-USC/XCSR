...
if [ "$MODE" = "pretrain" ]; then
    ...
    do 
        ...
        if [[ "$active_langs" == *"$lang"* ]]; then 
            ...
            CUDA_VISIBLE_DEVICES=6,7 python xcsr_experiments/run_mcqa.py \
                ...
                --prediction_output ${DATA_DIR}/results/mcp_self_test_xlmrb_result.npy
        fi 
    done
    ...
    MODEL_DIR=/path/to/saved_models_xcsqa/mcp_xlmrb_en   # TODO: add tokenizer_config.json special_tokens_map.json sentencepiece.bpe.model
    # cp /path/to/saved_models_xcsqa/mcp_xlmrb_en/tokenizer_config.json $MODEL_DIR/
    # cp /path/to/saved_models_xcsqa/mcp_xlmrb_en/special_tokens_map.json $MODEL_DIR/
    # cp /path/to/saved_models_xcsqa/mcp_xlmrb_en/sentencepiece.bpe.model $MODEL_DIR/
    ...
    do        
        ...
        do
            ...
            CUDA_VISIBLE_DEVICES=7 python xcsr_experiments/run_mcqa.py \
                ...
                --fp16 --overwrite_output \
                --overwrite_cache \
                --prediction_output ${DATA_DIR}/results/mcp_en-${lang}_${split}_xlmrb_result.npy
        done
    done
    ...
    do 
     ...
     echo "En-X zero-shot cross-lingual transfer" 
    declare -a langs=("en" "zh" "de" "es" "fr" "it" "jap" "nl" "pl" "pt" "ru" "ar" "sw" "ur" "vi" "hi")
    MODEL_DIR=/path/to/saved_models_xcodah/mcp_xlmrb_en   # TODO: add tokenizer_config.json special_tokens_map.json sentencepiece.bpe.model
    declare -a splits=("test" "dev")
    for lang in "${langs[@]}" 
    do        
        for split in "${splits[@]}" 
