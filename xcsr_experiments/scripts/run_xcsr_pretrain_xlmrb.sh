...
if [ "$MODE" = "pretrain" ]; then
    echo "Pre-train the xlmrb with XCSR probes" 
    DATA_DIR=corpus/XCSR
    MODEL_DIR=/mnt/nfs1/bill/saved_models_xlmrb_pretrained
    lr=5e-6
    ...
    do 
        ...
        if [[ "$active_langs" == *"$lang"* ]]; then 
            ...
            OUTPUT_DIR=/mnt/nfs1/bill/saved_models_xcsqa/xcsr_xlmrb_${lang}
            CUDA_VISIBLE_DEVICES=6,7 python methods/run_mcqa.py \
                --task_name xcsr \
                --exp_name "pted+xlmrb_${lang}|(${lr}-${warm})" \
                --model_name_or_path /mnt/nfs1/bill/saved_models_xlmrb_pretrained \
    ...
    MODEL_DIR=/mnt/nfs1/bill/saved_models_xcsqa/xcsr_xlmrb_en   # TODO: add tokenizer_config.json special_tokens_map.json sentencepiece.bpe.model
    # cp /mnt/nfs1/bill/saved_models_xcsqa/xcsr_xlmrb_en/tokenizer_config.json $MODEL_DIR/
    # cp /mnt/nfs1/bill/saved_models_xcsqa/xcsr_xlmrb_en/special_tokens_map.json $MODEL_DIR/
    # cp /mnt/nfs1/bill/saved_models_xcsqa/xcsr_xlmrb_en/sentencepiece.bpe.model $MODEL_DIR/
    ...
    do 
        ...
        if [[ "$active_langs" == *"$lang"* ]]; then 
            ...
            OUTPUT_DIR=/mnt/nfs1/bill/saved_models_xcodah/xcsr_xlmrb_${lang}
     ...
     echo "En-X zero-shot cross-lingual transfer" 
    declare -a langs=("en" "zh" "de" "es" "fr" "it" "jap" "nl" "pl" "pt" "ru" "ar" "sw" "ur" "vi" "hi")
    MODEL_DIR=/mnt/nfs1/bill/saved_models_xcodah/xcsr_xlmrb_en   # TODO: add tokenizer_config.json special_tokens_map.json sentencepiece.bpe.model
    declare -a splits=("test" "dev")
    for lang in "${langs[@]}" 
    do        
        for split in "${splits[@]}" 
